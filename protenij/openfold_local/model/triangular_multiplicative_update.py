...
from protenij.openfold_local.model.primitives import LayerNorm, Linear
from protenij.openfold_local.utils.precision_utils import is_fp16_enabled
from protenij.openfold_local.utils.tensor_utils import add, permute_final_dims
...
class TriangleMultiplicativeUpdate(BaseTriangleMultiplicativeUpdate):
    ...
    def _inference_forward(
        ...
        # [*, N, N, c]
        a = compute_projection(z, mask, True, chunked=True) # !!! TRUe

        # a_no_chunk = compute_projection(z, mask, True, chunked=False)

        # residual = a_no_chunk - a
        # res_max = residual.abs().max()
        # print(f"residual max: {res_max}")
        # print("F!")
        12

        if False:##inplace_chunk_size is not None:
            n = a.shape[-1]
            half_n = n // 2 + n % 2
            row_dim = -3
            col_dim = -2
    ...
    def forward(
        ...
        if inplace_safe:
            ...
            # x_no_chunk = self._inference_forward(
            #     z,
            #     mask,
            #     inplace_chunk_size=None,
            #     with_add=_add_with_inplace,
            # )
            # residual = x_no_chunk - x
            # res_max = residual.abs().max()
            # print(f"residual max: {res_max}")
            return x
