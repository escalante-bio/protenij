...
from protenij.data.infer_data_pipeline import get_inference_dataloader
...
from protenij.utils.torch_utils import to_device
from protenij.web_service.dependency_url import URL
from protenij.runner.dumper import DataDumper
...
class InferenceRunner(object):
    ...
    # Adapted from protenij.runner.train.Trainer.evaluate
    @torch.no_grad()
...
def download_infercence_cache(configs: Any) -> None:
    ...
    for cache_name in (
        ...
        print("current cache file path", cur_cache_fpath)
...
def main(configs: Any, interactive: bool=False):
    # Runner
...
def run(interactive=False):
    ...
    return main(configs, interactive=interactive)
